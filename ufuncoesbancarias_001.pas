...
interface
  uses
    ...
    Function BRADESCO_SR_CODIGOBARRAS(sBanco,sMoeda,sVencimento,sValor,sAgencia,sCarteira,sAno_NossoNro,sNossoNro,sConta : String):String;
    Function BRADESCO_SR_LINHADIGITAVEL(sBanco,sMoeda,sVencimento,sValor,sAgencia,sCarteira,sAno_NossoNro,sNossoNro,sConta : String):String;
    ...
    Function ITAU_SR_CODIGOBARRAS(sVencimento,sValor,sCampoLivre):string;
    Function ITAU_SR_LINHADIGITAVEL(sVencimento,sValor,sCampoLivre):string;
    ...
    Function SANTANDER_SR_CODIGOBARRAS(sBanco,sMoeda,sVencimento,sValor,sCarteira,sConta,sNossoNr):string;
    Function SANTANDER_SR_LINHADIGITAVEL(sBanco,sMoeda,sVencimento,sValor,sCarteira,sAgencia,sConta,sNossoNr):string;
    ...
    //funções BANCO DO BRASIL SEM REGISTRO (NOSSO NUMERO 17 BYTES (CONVENIO 6 BYTES))
    Function BRASIL_SR_CODIGOBARRAS(sBanco,sMoeda,sVencimento,sValor,sCarteira,sConta,sNossoNr):string;
    Function BRASIL_SR_LINHADIGITAVEL(sBanco,sMoeda,sVencimento,sValor,sCarteira,sAgencia,sConta,sNossoNr):string;
...
Begin
  ...
  sLivre  := sAgencia+sCarteira+sAno_NossoNro+sNossoNro+sConta+'0';

  //  WDAC_CB   -->  Digito Verificador [Codigo de Barras]   "MODULO 11"
  sDAC_CB := BRADESCO_SR_CALCULA_MODULO11(sBanco+sMoeda+sFator+sValor+sLivre);

  Result  := PChar(sBanco   + sMoeda   + sDAC_CB   + sFator   +
                   sValor   + sAgencia + sCarteira + sAno_NossoNro   +
                   sNossoNro  + sConta   + '0');
End;
...
Begin
  ...
  {-- Campo 4 ---------------------------------------------------------------}
  sCampo4 := BRADESCO_SR_CALCULA_MODULO11(sBanco+sMoeda+sFator+sValor+sLivre);
...
begin
  ...
  begin
    if bDig = True then
      iRes := StrToInt(Copy(sString,I,1)) * 2
    else
      iRes := StrToInt(Copy(sString,I,1)) * 1;
...
begin
  ...
  begin
    ...
    iDig := StrToInt(Copy(sNossoNr,I,1)) * iMult;

    if iDig >= 10 then
      iSoma := iSoma + StrToInt(SomaDigitos(IntToStr(iDig)))
    else
      iSoma := iSoma + iDig;
...
begin
  ...
  sDigito := CALCM11('3419' + sFator+sValor+sCampoLivre+ITAU_SR_MODULO10(sCampoLivre)+'0');
...
begin
  ...
  for iI := Length(sString) downto 1 do
  begin
    iRes := StrToInt(Copy(sString,I,1)) * iMult;
    iSoma := iSoma + iRes;
...
//  sDig   -->  Digito Verificador [Codigo de Barras]   "MODULO 11"
    sDig    := SANTANDER_SR_MODULO11(sBanco+sMoeda+sFator+sValor+'9'+sConta+
                                     sNossoNr+'0'+sCarteira);

    Result := (sBanco   + sMoeda    + sDig + sFator +
               sValor   + '9' + sConta + sNossoNr + '0' +
               sCarteira );
end;
...
//  sDig_CB   -->  Digito Verificador [Codigo de Barras]   "MODULO 11"
    sDig_CB    := SANTANDER_SR_MODULO11(sBanco+sMoeda+sFator+sValor+'9'+sConta+
                                        sNossoNr+'0'+sCarteira);
...
begin
  ...
  sFator  := IntToStr(GENERICO_FATOR_DE_VENCIMENTO(sVencimento));
  sLivre  := sNossoNr+sAgencia+sConta+sCarteira;
...
begin
  ...
  begin
    sNossoNr := sNossoNr + IntToStr(iAux);
  end;
  Result:=sNossoNr;
end;
